...
<div class="admin-card">
  ...
  <form method="POST" action="/admin/<?= $is_new ? 'new' : 'edit' ?><?= !$is_new ? '?path=' . urlencode($relPath) : '' ?>">
    ...
    <div style="margin-bottom:1.25rem">
      ...
      <textarea id="body" name="body"><?= htmlspecialchars($md_body, ENT_QUOTES) ?></textarea>
    </div>
...
<style>
  .sun-editor { border-radius: 5px; border-color: #d1d5db; }
  .sun-editor .se-toolbar { border-radius: 5px 5px 0 0; }
</style>
...
(function () {
  ...
  SUNEDITOR.create(document.getElementById('body'), {
    height: 500,
    buttonList: [
      ['undo', 'redo'],
      ['formatBlock'],
